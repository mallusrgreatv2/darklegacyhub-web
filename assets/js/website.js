...
select(".bg-op-page").style.backgroundColor =
  bg.opacity.color == undefined ? "black" : bg.opacity.color;
select(".bg-op-page").style.opacity =
  bg.opacity.value == undefined ? 0.5 : bg.opacity.value;
...
for (var i = 0; i < ButtonName.length; i++) {
  ...
  Text.style.background =
    ButtonStyle["background-color"] == undefined
      ? "#fff659"
      : ButtonStyle["background-color"];

  Text.style.color =
    ButtonStyle["text-color"] == undefined
      ? "black"
  ...
  if (ConfigB.icon != undefined)
    Icon.setAttribute("src", ConfigB.icon), Button.appendChild(Icon);
...
StatusButton.style.background =
  StatusConfig["background-color"] == undefined
    ? "#fff659"
    : StatusConfig["background-color"];
StatusButtonText.style.color =
  StatusConfig["text-color"] == undefined
    ? "black"
    : StatusConfig["text-color"];
...
ServerStatus(Config["server-ip"]).getData((server) => {
  ...
  const ButtonValue = StatusConfig["value"],
    getStatus = (s) =>
      s
        .replace(/{online}/g, IsOnline ? server.players.online : 0)
        .replace(/{max}/g, IsOnline ? server.players.max : 0);
